...
Причем $\sigma_{\bar x} = \sigma/\sqrt{n}$.
...
## Различается ли секреция тетрагидрокортизона при аденоме и двусторонней гиперплазии надпочечников?
...
Какая нужна выборка, чтобы обнаружить _сильный эффект_ с вероятностью 0.8 при уровне значимости 0.01?
...
```{r}
library(readxl)
flea <- read_excel(path = 'data/fleabeetles-subset.xlsx',
                   sheet = 'dat')
```
